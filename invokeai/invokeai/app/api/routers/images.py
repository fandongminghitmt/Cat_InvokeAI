    ...
    if file.content_type and file.content_type.startswith("video"):
        try:
            ...
            video_dto = ApiDependencies.invoker.services.videos.create(
                ...
                video_category=image_category, # Use the category passed (likely 'user' or 'general')
                session_id=session_id,
                board_id=board_id,
                is_intermediate=is_intermediate,
            )
            # We must return ImageDTO because that's what the frontend expects
            return convert_video_to_image_dto(video_dto)
