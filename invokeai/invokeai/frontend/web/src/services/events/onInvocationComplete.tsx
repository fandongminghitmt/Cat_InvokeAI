...
<<<<<<< HEAD
    ...
    console.log('[TraeDebug] onInvocationComplete Start', { currentSelectedBoardId, imageDTOsLength: imageDTOs.length });
    for (const dto of imageDTOs) {
      // eslint-disable-next-line no-console
      console.log('[TraeDebug] Processing DTO', { name: dto.image_name, board: dto.board_id, target: currentSelectedBoardId });
      if (currentSelectedBoardId && currentSelectedBoardId !== 'none' && dto.board_id !== currentSelectedBoardId) {
        // eslint-disable-next-line no-console
        console.log('[TraeDebug] MOVING image to', currentSelectedBoardId);
        imagesToAddToBoard.push(dto.image_name);
        dto.board_id = currentSelectedBoardId;
      } else {
        // eslint-disable-next-line no-console
        console.log('[TraeDebug] SKIPPING move', { reason: 'Same board or no target', conditions: { hasTarget: !!currentSelectedBoardId, targetNotNone: currentSelectedBoardId !== 'none', different: dto.board_id !== currentSelectedBoardId } });
      }
    }

    if (imagesToAddToBoard.length > 0 && currentSelectedBoardId && currentSelectedBoardId !== 'none') {
      dispatch(
        imagesApi.endpoints.addImagesToBoard.initiate({
          board_id: currentSelectedBoardId,
          image_names: imagesToAddToBoard,
        })
      );
    }
