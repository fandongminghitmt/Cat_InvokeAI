...
class VideoOutput(BaseInvocationOutput):
    ...
    fps: float = OutputField(description="The frame rate of the video")
...
@invocation_output("video_collection_output")
class VideoCollectionOutput(BaseInvocationOutput):
    """Base class for nodes that output a collection of videos"""

    collection: list[VideoField] = OutputField(
        description="The output videos",
    )
...
@invocation(
    "video_collection",
    title="Video Collection Primitive",
    tags=["primitives", "video", "collection"],
    category="primitives",
    version="1.0.0",
)
class VideoCollectionInvocation(BaseInvocation):
    """A collection of video primitive values"""

    collection: list[VideoField] = InputField(description="The collection of video values")

    def invoke(self, context: InvocationContext) -> VideoCollectionOutput:
        return VideoCollectionOutput(collection=self.collection)
...
@invocation_output("audio_output")
class AudioOutput(BaseInvocationOutput):
    """Base class for nodes that output a single audio"""

    audio: AudioField = OutputField(description="The output audio")
    duration: float = OutputField(description="The duration of the audio in seconds")

    @classmethod
    def build(cls, audio_dto: AudioDTO) -> "AudioOutput":
        return cls(
            audio=AudioField(audio_name=audio_dto.audio_name),
            duration=audio_dto.duration,
        )


@invocation_output("audio_collection_output")
class AudioCollectionOutput(BaseInvocationOutput):
    """Base class for nodes that output a collection of audios"""

    collection: list[AudioField] = OutputField(
        description="The output audios",
    )


@invocation("audio", title="Audio Primitive", tags=["primitives", "audio"], category="primitives", version="1.0.0")
class AudioInvocation(BaseInvocation):
    """An audio primitive value"""

    audio: AudioField = InputField(description="The audio to load")

    def invoke(self, context: InvocationContext) -> AudioOutput:
        # Placeholder implementation
        # audio_dto = context.audios.get_dto(self.audio.audio_name)
        # return AudioOutput.build(audio_dto=audio_dto)
        pass


@invocation(
    "audio_collection",
    title="Audio Collection Primitive",
    tags=["primitives", "audio", "collection"],
    category="primitives",
    version="1.0.0",
)
class AudioCollectionInvocation(BaseInvocation):
    """A collection of audio primitive values"""

    collection: list[AudioField] = InputField(description="The collection of audio values")

    def invoke(self, context: InvocationContext) -> AudioCollectionOutput:
        return AudioCollectionOutput(collection=self.collection)
